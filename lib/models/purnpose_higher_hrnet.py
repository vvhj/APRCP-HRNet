...
class PosePurnHigherResolutionNet(nn.Module):
    def __init__(self, cfg, bncfg=None,**kwargs):
        self.inplanes = 64
        extra = cfg.MODEL.EXTRA
        super(PoseHigherResolutionNet, self).__init__()
        self.bncfg = bncfg
        self.bnindex = 0
        #PoseHigherResolutionNet.bnindex = bnindex
        # stem net
    ...
    def _make_deconv_layers(self, cfg, input_channels):
        ...
        for i in range(deconv_cfg.NUM_DECONVS):
            ...
            for _ in range(cfg.MODEL.EXTRA.DECONV.NUM_BASIC_BLOCKS):
                layers.append(nn.Sequential(
                    BasicBlock(output_channels, output_channels,self.bncfg[self.bnindex:self.bnindex+2]),
                ))
                self.bnindex += 2
...
def get_pose_net(cfg, bncfg,is_train, **kwargs):
    model = PosePurnHigherResolutionNet(cfg, bncfg,**kwargs)
